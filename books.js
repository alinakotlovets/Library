...
function Valid(item) {
  ...
  if (item.checkValidity() === false) {
    if (item.validity.valueMissing) {
      item.setCustomValidity(`${item.id} is missing`);
    } else if (item.type === "number" && item.validity.badInput) {
      item.setCustomValidity(`${item.id} must be a number`);
    } else if (item.type === "number" && item.validity.rangeUnderflow) {
      item.setCustomValidity(`${item.id} can't be less than 1`);
    }
    item.reportValidity();
    return false;
  }
...
form.addEventListener("submit", function (event) {
  ...
  if (!Valid(author)) return;
  if (!Valid(pages)) return;
